...
export const createReport = (name, subject, email, message) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post('/api/v1/reports', { name, subject, email,  message}, config);
...
export const updateReport = (id, name, subject, email, message) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(`/api/v1/reports/${id}`, { name, subject, email, message }, config);
